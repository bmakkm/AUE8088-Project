...
class LoadRGBTImagesAndLabels(LoadImagesAndLabels):
    ...
    def load_mosaic(self, index):
        ...
        lwir4 = np.full((s * 2, s * 2, 3), 114, dtype=np.uint8)  # base lwir image with 4 tiles (3 channels)
        img4 = np.full((s * 2, s * 2, 3), 114, dtype=np.uint8)  # base RGB image with 4 tiles

        for i, index in enumerate(indices):
            # Load images for both modalities
            imgs, _, img_shapes = self.load_image(index)
            lwir, img = imgs[0], imgs[1]  # Both lwir and img are 3 channels
            h, w = img_shapes[0]  # Assuming both modalities have the same shape
            # print("h,w", h, w)
            # Place img in img4 and lwir in lwir4
            if i == 0:  # top left
            ...
            lwir4[y1a:y2a, x1a:x2a] = lwir[y1b:y2b, x1b:x2b]
        ...
        # Apply random perspective once and get the transformation matrix
        img4, labels4, segments4 = copy_paste(img4, labels4, segments4, p=self.hyp["copy_paste"])
        lwir4, labels4, segments4 = copy_paste(lwir4, labels4, segments4, p=self.hyp["copy_paste"])
        img4, labels4, matrix = random_perspective(
            img4,
            labels4,
            degrees=self.hyp["degrees"],
            translate=self.hyp["translate"],
        ...
        # Apply the same transformation to lwir4 using the matrix
        lwir4 = cv2.warpPerspective(lwir4, matrix, (img4.shape[1], img4.shape[0]))
    
        return [lwir4, img4], labels4
